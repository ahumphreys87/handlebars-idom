  ...
  if (blockStatement.params.length) {
    ...
    switch (type) {
      case 'if':
        ...
        break;
      case 'unless':
        emitter.unless(blockStatement.params, function() {
          walkTree(blockStatement.program, emitter, iteree);
        });
        break;
      case 'each':
...
function walkElementNode(elementNode, emitter, iteree) {
  ...
  if (elementNode.children.length || !skipAttr) {
    ...
    emitter.placeholder(elementNode.tag, elementNode.attributes);
  }
}
...
function walkComponentNode(componentNode, emitter, iteree) {
  ...
  if (!skipAttr) {
    emitter.element(componentNode.tag, componentNode.attributes, function() {
      walkTree(componentNode.program, emitter, iteree);
    });
  } else {
    emitter.placeholder(componentNode.tag, componentNode.attributes);
  }
}
